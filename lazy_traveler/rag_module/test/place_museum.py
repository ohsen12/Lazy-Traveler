...
def search_places(location, lat, lng, type, category):
    '''
    Google Places API를 사용하여 특정 위치(location)의 위도·경도를 기준으로 반경 내 해당 type의 장소를 검색한 후, 
    중복을 제외하고 각 장소의 세부 정보를 가져와 리스트로 반환하는 함수.
    '''
    try:
...
def main():
    print("\n📝 종로구 반경 내에서 특정 타입의 장소를 찾아 문서화하는 프로그램\n")
    ...
    for location in locations:
        ...
        # 💊
        print(repr(f"\n📍 {location} (위도: {lat}, 경도: {lng}) 검색 시작.."))
        
        # search_places를 사용하여 해당 location 내에서 각 type 별 장소들을 검색해서 문서화하기
        for type, category in type_category_map.items():
            print(f"\n🔍 {category} ({type}) 검색 중..")
            try:
                # ex. 세종문화회관의, park 타입 장소 검색. 카테고리는 공원으로 설정.
            ...
            if isinstance(results, list):
                ...
                if not results:
                    # 결과가 없으면 종료
                    print(f"{location} . '{category}' 카테고리의 장소가 더 이상 없습니다. 검색을 종료합니다.")
                    break  # 종료조건
                
                for p in results:
                    # 종로구이며, 영업시간 정보가 있고, 리뷰 개수가 30 이상인 장소만 문서에 추가
                    #❗️조정❗️
                    if "종로구" in p['주소'] and p['영업시간'] != "정보 없음" and int(p['리뷰 개수'])>=30 and "미술관" not in p['이름'] and "갤러리" not in p['이름']:
